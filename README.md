...
- **"Your turn" section and visitor data.** `components/Reactions` ends the page for visitors arriving from social media. It opens with a combination lock (`Lock.jsx`): the number is `SPECIAL_NUMBER` and the message it reveals `SPECIAL_MESSAGE`, both Vercel env vars on `portfolio-api`, checked only on the server (8 tries per 15 min per client, 30 per minute overall); opening it, and the reply written after, email me. Then an emoji slider (how did it land, 0 to 100, with the live average), a one-tap "what brought you here", a one-line note that is emailed to me and never shown, and the latest reactions (flag and emoji only). `backend/visitors.js` stores anonymous visits, events, reactions and notes in the `portfolio` schema (`backend/schema.sql`) through `DATABASE_URL`; no names, emails or IPs. Without `DATABASE_URL` the routes answer 503 and the page hides the shared numbers. Instagram link: `/?from=ig#react`.
