...
const SEED_IRELAND = [
  ...
  { id: "ie31", phase: "3 · Apply — remote-first bridge", name: "Apply to remote roles at Irish/EU companies you can do from KSA today", done: false,
    note: "This is the bridge: income now, Irish references, and the inside track to sponsorship later. Ask me to search for openings, or use the links below." },
 ...
 */
const SEED_ROADMAP = [
  { id: "r01", phase: "Every day — all 12 weeks", name: "3 job applications", done: false,
  ...
  return (
    <div className="hq">
      ...
      <nav className="hq-tabs">
        {[["jobs", "My Pipeline"], ["ireland", "🇮🇪 Ireland"]].map(([k, t]) => (
          <button key={k} className={tab === k ? "on" : ""} onClick={() => setTab(k)}>{t}</button>
        ))}
      </nav>

      <div className="hq-tabpanel" key={tab}>
        {tab === "jobs" && <Jobs data={data} setData={setData} counts={counts} followUpIds={followUpIds} />}
...
function Jobs({ data, setData, counts, followUpIds }) {
...
/* ============== shared: phase-grouped checklist (used by the Ireland plan) ============== */
